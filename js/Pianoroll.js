			...
			if (ins=="audi") {
				...
				if (self.lastKey!=tickY && self.selCount()==0){
					...
					if (ins) ins.triggerAttackRelease(
						...
						Tone.Time("8n").toSeconds(), Tone.now(), 1
					);					
					self.lastKey=tickY;
				};
			} else	
		...
		if (self.currentSelCount==0) {
			...
			if (theNote!=undefined) {
				...
				if (ins) ins.triggerAttackRelease(
					...
					Tone.now(), Work.global.seqXY[theNote].v
				);
			} else {
...
// 				self.playingFromT=Tone.now();
				...
				if (!self.isPlaying && (Controls.tempDrag=="" ? self.dragType : Controls.tempDrag)!="audi") {
					...
					if (ins) ins.triggerAttackRelease(
						Global.chromatic_scale[Work.global.scaledKeyboard?Composer.scale[tickY]:tickY],
						Tone.Time("8n").toSeconds(), Tone.now(), 0.5
					);					
				};
			}			
		} else {
...
//			console.log("improvising");
	...
	if (Work.global.metronome){
		var tt=(this.playTick % tickPerMeas);
 		if (tt==0 && this.playTick<this.selEnd) Instruments.drum2.triggerAttackRelease(0.001, t, 0.1*Global.metroVolume);
		...
		if ( (Work.global.seqIJ[currentTick].notes[i].note<88 && 
			...
			if (pedal==1) {		
				if (ins!=null) ins.triggerAttack(
					...
					Work.global.seqIJ[currentTick].notes[i].vel*(1+(Math.random()-0.5)*Work.global.human_vel)
				);
				this.lastPedal=1;
			} else {
